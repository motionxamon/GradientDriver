/*
GradientDriver.jsx
Apply Ramp-driven gradient control to selected After Effects properties.
...
(function gradientDriver() {
    ...
    function driverExpression(ctrlName, gradientName, minName, maxName, info) {
        var base = '' +
            ...
            'r=grad.effect("Gradient Driver Ramp");\n' +
            'p=thisLayer.toComp(anchorPoint);\n' +
            'a=r(1); b=r(3);\n' +
            'c1=r(2); c2=r(4);\n' +
            'vec=b-a;\n' +
            'den=Math.max(0.0001,vec[0]*vec[0]+vec[1]*vec[1]);\n' +
            'isRadial=false;\n' +
            'try { isRadial=(r(5)==2); } catch (err) {}\n' +
            'if (isRadial) {\n' +
            '  dx=p[0]-a[0]; dy=p[1]-a[1];\n' +
            '  u=Math.sqrt(dx*dx+dy*dy)/Math.sqrt(den);\n' +
            '} else {\n' +
            '  u=((p[0]-a[0])*vec[0]+(p[1]-a[1])*vec[1])/den;\n' +
            '}\n' +
            'u=clamp(u,0,1);\n' +
            'l1=(c1[0]+c1[1]+c1[2])/3;\n' +
            'l2=(c2[0]+c2[1]+c2[2])/3;\n' +
